;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;; Defines a virtual reality actor which can be entered into a trial scene.

(in-package #:org.shirakumo.fraf.trial.vr)

(defclass actor ()
  ((name :initarg :actor)))
 
(defmethod trial:enter ((unit actor) scene)
  (let* ((head (make-instance 'head))
         (left-render-pass (make-instance 'left-eye-render-pass))
         (right-render-pass (make-instance 'right-eye-render-pass))
         (compositor-render-pass (make-instance 'compositor-render-pass))
         (ui (make-instance 'dui :target-resolution (alloy:px-size 800 600)))
         (ui-render-pass (make-instance 'ui-render-pass :overlay-key "First overlay" :dui ui))
         (focus-1 (make-instance 'alloy:focus-list :focus-parent (alloy:focus-tree ui)))
         (layout-1 (make-instance 'alloy:vertical-linear-layout
                                  :layout-parent (org.shirakumo.alloy:layout-tree ui))))
    (let* ((data (3d-vectors:vec2 0 1))
           (vec (org.shirakumo.alloy:represent data 'trial-alloy::vec2
                                               :focus-parent focus-1 :layout-parent layout-1)))
      (alloy:on (setf alloy:value) (value vec)
        (print value)))
    (trial:enter head scene)
    (trial:enter left-render-pass scene)
    (trial:enter right-render-pass scene)
    (trial:enter compositor-render-pass scene)
    (trial:enter ui-render-pass scene)
    (trial:enter ui scene)
    (trial:connect (trial:port left-render-pass 'trial:color)
                   (trial:port compositor-render-pass 'left-pass-color)
                   scene)
    (trial:connect (trial:port right-render-pass 'trial:color)
                   (trial:port compositor-render-pass 'right-pass-color)
                   scene)
    (trial:connect (trial:port left-render-pass 'trial:depth)
                   (trial:port compositor-render-pass 'left-pass-depth)
                   scene)
    (trial:connect (trial:port right-render-pass 'trial:depth)
                   (trial:port compositor-render-pass 'right-pass-depth)
                   scene)
    (trial:connect (trial:port ui-render-pass 'trial:color)
                   (trial:port compositor-render-pass 'ui-pass)
                   scene)
    (vr:create-overlay "Second overlay" "abc")
    (vr:create-overlay "Third overlay" "abc")
    (vr:create-overlay "Fourth overlay" "abc")
    (set-overlay-displacement "Second overlay" 0.1 0.1 0.1)
    (set-overlay-displacement "Third overlay" 0.2 0.2 0.2)
    (set-overlay-displacement "Fourth overlay" 0.3 0.3 0.3)
    (set-overlay-visibility t "First overlay")
    (set-overlay-visibility t "Second overlay")
    (set-overlay-visibility t "Third overlay")
    (set-overlay-visibility t "Fourth overlay")    
    (vr:set-overlay-texture-bounds (vr:find-overlay "First overlay") 0.0 0.0 0.2 0.2 )
    (vr:set-overlay-texture-bounds (vr:find-overlay "Second overlay") 0.1 0.0 0.5 0.5 )
    (vr:set-overlay-texture-bounds (vr:find-overlay "Third overlay") 0.0 0.0 0.3 0.3 )
    (vr:set-overlay-texture-bounds (vr:find-overlay "Fourth overlay") 0.0 0.0 0.4 0.4  )))
